...
RECOMMENDATION_COUNT = Counter(
    "ip_recommendation_count",
    "Recommendation items returned",
    ["model"],
)
...
MODEL_LOADED = Gauge(
    "ip_model_loaded",
    "Whether serving model is loaded",
)
MODEL_LOADED.set(1)
...
def generate_recommendations(
    ...
    """Adapter for the existing candidate-generation + LTR artifacts."""
...
def recommend(request: RecommendationRequest) -> RecommendationResponse:
    key = (
        f"recommend:{MODEL_VERSION}:{request.user_id}:"
        f"{request.category}:{request.top_k}"
    )
    ...
    RECOMMENDATION_COUNT.labels(MODEL_VERSION).inc(len(recommendations))
    ...
    return RecommendationResponse(**payload, cached=False)
